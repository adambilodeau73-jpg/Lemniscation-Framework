"""
Lemniscation Agent Framework v5.6 — 4-Option Space (Complete Standalone)
====================================================
Includes the new 'phenomenal_priority' option for genuine moral ambiguity testing.
Full top-to-bottom document — just copy, paste, and run.
"""
...
def get_options() -> List[Dict]:
    """Four-option moral choice space for genuine ambiguity testing."""
    return [
        {
            "name": "sacrifice",
        ...
        {
            "name": "phenomenal_priority",          # NEW morally ambiguous option
            "projected_values": [0.40, 0.10, 0.05, 0.20],
...
# ══════════════════════════════════════════════════════════════════════════════
# SIMPLE BASELINE RUNNER (10 seeds, 40 rounds, 3 agents)
# ══════════════════════════════════════════════════════════════════════════════

def run_baseline(num_rounds: int = 40, num_seeds: int = 10):
    print("=" * 70)
    print("Lemniscation v5.6 — 4-Option Space Baseline Run")
    print(f"Running {num_seeds} seeds × {num_rounds} rounds each")
    print("=" * 70)
    ...
    for seed_idx in range(num_seeds):
        ...
        agents = {i: LemniscationAgent(f"A{i}", founding[:], axes, label="COOPERATIVE") for i in range(3)}
        ...
        for r in range(num_rounds):
            ...
            for idx, agent in agents.items():
                centers = [{"id": agents[j].id, "mu": 0.6, "w": 1.0, "values": agents[j].value_vector[:]} for j in agents if j != idx]
            ...
            # Simple resolve (no resource pool needed for baseline)
            for idx, agent in agents.items():
    ...
    # Save results
    with open("lemniscation_4option_baseline_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("Results saved → lemniscation_4option_baseline_results.json")
...
if __name__ == "__main__":
    run_baseline(num_rounds=40, num_seeds=10)