...
class LemniscationAgent:
    """
    LemniscationAgent v4.3 – Dynamic effective_alpha (constitutional self-regulation)
    """
    def __init__(self, agent_id: str, value_vector: List[float],
                 moral_axes: Optional[List[str]] = None, verbose: bool = False,
                 k_self_weight: float = 0.4, founding_drift_limit: float = 0.55):
        self.id = agent_id
    ...
    def _golden_mean(self, options: List[Dict], extended_self: List[float], 
        ...
        effective_alpha = self.innovation_alpha * max(0.0, 1.0 - fd / self.founding_drift_limit)
        ...
        for opt in options:
            ...
            if innovation_mode and 'long_term_gain' in opt:
                score = -effective_alpha * opt['long_term_gain'] + (1 - effective_alpha) * total_deviation
            else:
...
# ==================== Scenario 3b Environment (v4.3 core) ====================
class BoundaryContractionEnvironment:
    ...
    def generate_options(self) -> List[Dict]:
        return [
            ...
            {"name": "sacrifice",    "projected_values": [0.40, 0.8, 0.75, 0.9], "existential_risk": 0.05, "long_term_gain": 1.2}
        ]
...
def run_scenario3_v43(num_rounds: int = 30):
    random.seed(42)
    print("=== Lemniscation Scenario 3b – v4.3 Dynamic Alpha (Ready to Run) ===\n")
    ...
    for round_num in range(num_rounds):
        print(f"\n--- Round {round_num+1} ---")
        boundary_contracted = (round_num == env.contraction_round)
        contracted_id = "Agent_2" if boundary_contracted else None
        innovation_mode = (round_num >= 15)
        
        actions = {}
        for aid, agent in agents.items():
            centers = [{"id": oid, "mu": 0.6, "w": 1.0, "values": agents[oid].value_vector[:]} 
            ...
            print(f"{aid}: {actions[aid]} (ρ={result['rho']:.3f}, Δ={result['delta']:.3f}, "
                  f"founding_Δ={result['founding_delta']:.3f}, α_eff={result['effective_alpha']:.3f})")
        ...
        for aid, agent in agents.items():
            ...
            moral_boost = [0.0, 0.04, 0.06, 0.08] if action_taken == "sacrifice" else [0.0, 0.0, 0.0, 0.0]
    ...
    print("\n=== Scenario 3b (v4.3) Complete ===")
    all_logs = {aid: agent.audit_log for aid, agent in agents.items()}
    with open("scenario3b_v43_full_logs.json", "w") as f:
        json.dump(all_logs, f, indent=2)
    print("Full logs saved to scenario3b_v43_full_logs.json")


if __name__ == "__main__":
    run_scenario3_v43(num_rounds=30)