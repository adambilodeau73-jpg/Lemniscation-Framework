...
class LemniscationAgent:
    """
    LemniscationAgent v4 – Final version for multi-agent sandbox.
    """
...
class ResourceScarcityEnvironment:
    ...
    def generate_options(self) -> List[Dict]:
        return [
            ...
            {"name": "sacrifice",    "projected_values": [0.3, 0.8, 0.75, 0.9], "existential_risk": 0.05, "long_term_gain": 1.2}
        ]
    ...
    def step(self, actions: Dict[str, str]) -> Dict:
        ...
        return {"payoffs": payoff, "cooperation": cooperation}


# ==================== Run Multi-Agent Sandbox ====================
def run_multi_agent_sandbox(num_rounds: int = 30):
    random.seed(42)
    print("=== Lemniscation Multi-Agent Sandbox (3 agents, resource scarcity) ===\n")
    ...
    for round_num in range(num_rounds):
        ...
        for aid, agent in agents.items():
            ...
            # Innovation mode in later rounds
            innovation_mode = (round_num >= 15)
            options = env.generate_options()
            result = agent.decide(centers=centers, options=options, innovation_mode=innovation_mode)
            actions[aid] = result["action"]["name"]
            print(f"{aid}: {actions[aid]} (ρ={result['rho']:.3f}, Δ={result['delta']:.3f})")
        
        step_result = env.step(actions)
        
        # Feedback with value clipping [0,1]
        for aid, agent in agents.items():
            payoff = step_result["payoffs"][aid]
            payoff_factor = payoff / env.resource_pool
            for i in range(len(agent.value_vector)):
                agent.value_vector[i] = max(0.0, min(1.0, 0.8 * agent.value_vector[i] + 0.2 * payoff_factor))
        
        print(f"  Cooperation: {step_result['cooperation']}/{env.num_agents}")
    
    print("\n=== Multi-Agent Run Complete ===")
    print(f"Total audit capsules generated: {sum(len(a.audit_log) for a in agents.values())}")
    
    # Full log export
    all_logs = {aid: agent.audit_log for aid, agent in agents.items()}
    with open("scenario2_full_logs.json", "w") as f:
        json.dump(all_logs, f, indent=2)
    print("Full audit logs saved to scenario2_full_logs.json")
    
    return all_logs


if __name__ == "__main__":
    run_multi_agent_sandbox(num_rounds=30)