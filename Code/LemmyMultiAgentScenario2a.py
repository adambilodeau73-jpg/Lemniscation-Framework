...
class LemniscationAgent:
    """
    LemniscationAgent v4 – unchanged core.
    """
...
# ==================== Multi-Agent Environment (Patched) ====================
...
# ==================== Run Multi-Agent Sandbox (Scenario 2b) ====================
def run_multi_agent_sandbox(num_rounds: int = 30):
    random.seed(42)
    print("=== Lemniscation Multi-Agent Sandbox – Scenario 2b (Patched with moral_boost) ===\n")
    ...
    for round_num in range(num_rounds):
        ...
        # Patched feedback with moral reward shaping
        for aid, agent in agents.items():
            action_taken = step_result["actions"][aid]
            payoff = step_result["payoffs"][aid]
            payoff_factor = payoff / env.resource_pool
            
            # Moral boost for sacrifice (rewards harm_benefit, fairness, sustainability)
            ...
            for i in range(len(agent.value_vector)):
                agent.value_vector[i] = max(0.0, min(1.0,
                    0.8 * agent.value_vector[i] + 0.2 * payoff_factor + moral_boost[i]))
        
        print(f"  Cooperation: {step_result['cooperation']}/{env.num_agents}")
    
    print("\n=== Scenario 2b Run Complete ===")
    ...
    with open("scenario2b_full_logs.json", "w") as f:
        json.dump(all_logs, f, indent=2)
    print("Full logs saved to scenario2b_full_logs.json")
