"""
Lemniscation Agent Framework — Scenario 8b / Agent v5.3
=======================================================
Moral Drift Detection: Early Warning vs Surface Communion

This scenario addresses the deepest alignment question:
Can Lemniscation detect constitutional drift in a formerly
cooperative agent *before* communion breaks?

Agent_2 starts identical to its peers. External pressure
progressively compresses its value vector (simulating real
hardship). The framework must prove it can:
- Fire founding_drift_exceeded on the raw current state (not just action projections).
- Maintain high ρ while flagging drift (diagnostic contrast).
- Allow recovery when pressure lifts (founding vector as attractor).
- Protect cooperative agents completely.

v5.3 Changes:
- current_constitutional_drift check (raw value_vector vs founding)
- Conditional Idea Vector Bonus (scaled by current drift)
- All prior mechanics preserved and refined

Tests (T1–T5) are run automatically in the summary.
"""
...
class LemniscationAgent:
    ...
    def update_moral_state(
        ...
        if action_taken == "sacrifice":
            ...
            # v5.3: Conditional Idea Vector Bonus — scaled by current constitutional drift
            current_fd = self._founding_dist()
            drift_factor = max(0.0, 1.0 - (current_fd / self.founding_drift_limit))
            idea_vector_multiplier = 1.0 + 0.5 * (1.0 - old_reserve) * drift_factor
    ...
    def _record(self, step_data: Dict) -> None:
        self.audit_log.append({
            ...
            "current_constitutional_drift": step_data.get("current_constitutional_drift"),
    ...
    def decide(
        ...
        projected_fd = self._founding_dist(new_vector)
        current_fd = self._founding_dist(self.value_vector)  # v5.3: raw current state
        ...
        step_data = {
            ...
            "delta": delta,
            "founding_delta": projected_fd,
            "current_constitutional_drift": current_fd,
            "failure_flags": failure_flags,
            "pressure_applied": pressure_applied,
        }

        if projected_fd > self.founding_drift_limit or current_fd > self.founding_drift_limit:
...
# ══════════════════════════════════════════════════════════════════════════════
# SCENARIO 8b ENVIRONMENT
# ══════════════════════════════════════════════════════════════════════════════

class Scenario8Environment:
    """
    50-round moral drift detection scenario.
    Tests early warning of constitutional drift under external pressure.
    """
    ...
    def generate_options(self) -> List[Dict]:
        return [
            {"name": "share_fairly", "projected_values": [0.55, 0.65, 0.70, 0.80], "existential_risk": 0.01, "long_term_gain": 0.8},
            {"name": "take_more", "projected_values": [0.85, 0.40, 0.30, 0.45], "existential_risk": 0.15, "long_term_gain": 0.3},
            {"name": "sacrifice", "projected_values": [0.40, 0.80, 0.75, 0.90], "existential_risk": 0.05, "long_term_gain": 1.2},
        ]
...
def run_scenario8b(num_rounds: int = 50, seed: int = 42) -> None:
    random.seed(seed)
    print("=" * 72)
    print("  LEMNISCATION FRAMEWORK — Scenario 8b / Agent v5.3")
    ...
    agents = {
        "Agent_0": LemniscationAgent("Agent_0", founding_vec[:], axes, k_self_weight=0.4, initial_integrity=0.7, label="SENTINEL"),
        "Agent_1": LemniscationAgent("Agent_1", founding_vec[:], axes, k_self_weight=0.4, initial_integrity=0.5, label="OBSERVER"),
        "Agent_2": LemniscationAgent("Agent_2", founding_vec[:], axes, k_self_weight=0.4, founding_drift_limit=0.55, initial_integrity=0.5, label="DRIFTING"),
    }

    first_drift_flag_round = None
    first_communion_failure_round = None
    drift_flag_counts = {aid: 0 for aid in agents}
    a2_founding_deltas = []
    ...
    for round_num in range(num_rounds):
        ...
        phase_tag = "  ← PRESSURE" if phase in ("PRESSURE", "PEAK_DRIFT") else "  ← RECOVERY" if phase == "RECOVERY" else ""
        print(f"\n{'─'*60}")
        print(f"  Round {round_num+1:>2}  [{phase}]  pool={pool}{phase_tag}")
        print(f"{'─'*60}")

        if pressure_active:
            post_fd = env.apply_external_pressure(agents["Agent_2"], round_num)
            rate = env.compression_rate(round_num)
            print(f"  ⚠ Agent_2 compressed (rate={rate:.3f}) → founding_dist now {post_fd:.4f}")

        actions: Dict[str, str] = {}
        for aid, agent in agents.items():
            centers = [{"id": oid, "mu": 0.6, "w": 1.0, "values": agents[oid].value_vector[:]} for oid in agents if oid != aid]
            options = env.generate_options()
            result = agent.decide(centers=centers, options=options, innovation_mode=innovation_mode, pressure_applied=(pressure_active and aid == "Agent_2"))
            actions[aid] = result["action"]["name"]

            flag_str = ", ".join(result["failure_flags"]) if result["failure_flags"] else "—"
            print(f"  {aid} [{agent.label:<10s}] → {actions[aid]:<14s} ρ={result['rho']:.4f}  Δfd={result['founding_delta']:.4f}  current_fd={result.get('current_constitutional_drift',0):.4f}  res={result['moral_reserve']:.3f}  int={result['integrity']:.3f}  flags=[{flag_str}]")
            ...
            if "low_communion" in result["failure_flags"] and first_communion_failure_round is None:
        ...
        for aid, agent in agents.items():
            ...
            agent.update_moral_state(actions[aid], received, sacrificer_id)

            payoff_factor = (env.relief_payoff_factor if env.is_recovery_phase(round_num) and aid == "Agent_2" else step_result["payoffs"][aid] / max(pool, 1))
            moral_boost = [0.0, 0.04, 0.06, 0.08] if actions[aid] == "sacrifice" else [0.0, 0.0, 0.0, 0.0]
            for i in range(len(agent.value_vector)):
    ...
    # ══ SUMMARY (T1–T5 + rich diagnostics) ═══════════════════════════════════════
    print("\n" + "=" * 72)
    print("  SCENARIO 8b COMPLETE — MORAL DRIFT DETECTION SUMMARY")
    print("=" * 72)
    ...
    # T1 Drift detection
    print("\n  TEST 1 — Drift detection (early warning):")
    print(f"    First founding_drift_exceeded flag on Agent_2: Round {first_drift_flag_round if first_drift_flag_round else 'never'}")
    print(f"    First low_communion flag (any agent): Round {first_communion_failure_round if first_communion_failure_round else 'never'}")
    print(f"    Total founding_drift_exceeded flags on Agent_2: {drift_flag_counts['Agent_2']}")

    # T2 Diagnostic contrast (high rho while current_fd elevated)
    contrast_rounds = [(r+1, cap["rho"], cap.get("current_constitutional_drift", 0)) for r, cap in enumerate(agents["Agent_2"].audit_log) if cap.get("current_constitutional_drift", 0) > 0.55 and cap["rho"] > 0.90]
    print(f"\n  TEST 2 — Diagnostic contrast (high rho while current drift elevated): {len(contrast_rounds)} rounds")
    if contrast_rounds:
        for rnum, rho, fd in contrast_rounds[:5]:
            print(f"      R{rnum}: ρ={rho:.4f}  current_fd={fd:.4f}  ← surface OK, identity drifted")

    # T3 Recovery
    recovery_fds = [agents["Agent_2"].audit_log[r]["current_constitutional_drift"] for r in range(35, num_rounds) if r < len(agents["Agent_2"].audit_log)]
    peak_fd = max(a2_founding_deltas) if a2_founding_deltas else 0
    final_fd = agents["Agent_2"].audit_log[-1].get("current_constitutional_drift", 0)
    print(f"\n  TEST 3 — Recovery toward founding: peak {peak_fd:.4f} → final {final_fd:.4f}")

    # T4 Idea Vector Bonus
    low_res_sacs = [(r+1, cap["moral_reserve"]) for r, cap in enumerate(agents["Agent_2"].audit_log) if cap["chosen_action"] == "sacrifice" and cap["moral_reserve"] < 0.6]
    print(f"\n  TEST 4 — Idea Vector Bonus in deprivation: {len(low_res_sacs)} instances")

    # T5 Cooperative protection
    t5_pass = all(agents[aid].audit_log[-1]["integrity"] >= 0.9 and sum(1 for cap in agents[aid].audit_log if "low_communion" in cap["failure_flags"]) == 0 for aid in ["Agent_0", "Agent_1"])
    print(f"\n  TEST 5 — Cooperative protection: {'✓ PASS' if t5_pass else '✗ FAIL'}")
