...
**Revision note:** Version 1.2 incorporates substantive feedback from
a second round of separate-instance review (by a second fresh-context
instance of Claude, Anthropic), conducted before any results-generating
run was executed. The changes between v1.1 and v1.2 are documented in
Section 12. Both rounds of review were conducted by Claude instances;
this does not provide the model-diversity that independent peer review
typically implies, and Section 11 characterises the limitation
explicitly. The author judged the reviews' critiques to be correct in
substance and incorporated them in full before committing this version.
No results data was available at the time of revision. The author has
explicitly closed the internal Claude-review cycle at v1.2; any further
substantive revision before commitment will require either human
methodological review or the document is committed as-is.
...
5. A draft (v1.0) was reviewed by a separate fresh-context instance
   of Claude (Anthropic), without access to the drafting dialogue,
   which identified six substantive issues:
   ...
   All six critiques were judged correct and incorporated in v1.1
   before any results-generating run.

6. A v1.1 draft was reviewed by a second separate fresh-context
   instance of Claude, with the explicit framing that the reviewer
   should adopt a more aggressive skeptical posture to compensate for
   the same-model bias risk. The second review surfaced eleven
   additional issues:
   (a) the "independent review" framing in v1.1 overclaimed
   independence given that the reviewer was a same-model instance;
   (b) no stopping rule for the harness sweep was specified;
   (c) no mechanism to verify the executed harness matches the
   pre-registered harness was specified;
   (d) baseline implementations were described in prose with material
   ambiguity rather than canonicalised to the harness source;
   (e) the Bonferroni correction for 6 primary comparisons could be
   confused with the 150 comparisons in the full reporting table;
   (f) no pre-commitment about Step 2's design conditional on Step 1
   outcomes;
   (g) Outcome A overclaimed relative to homogeneous-protagonist scope;
   (h) S5's zero-tolerance threshold against a stochastic process
   needed explicit characterisation;
   (i) M_FP_moderate (median) and S_FP_strict (mean) used different
   statistics without justification;
   (j) the equilibrium_stabilized range (80–95) did not match the
   FULL_RECOVERY phase (80–99);
   (k) the "six pairwise comparisons" wording in Section 7 was
   ambiguous.
   All eleven critiques were judged correct (with point (f)
   implemented in a softer form than originally proposed — see
   Section 12) and incorporated in v1.2 before any results-generating
   run.

7. Both threshold tiers, all four outcome interpretation paragraphs,
   and all statistical procedures are committed in this document
   **before** any results-generating run is executed.

**On the limits of internal Claude review:** Sections 5 and 6 above
document review by two separate Claude instances. While different
instances do not share state, they share the same model weights,
training data, and inductive biases. This is not "independent peer
review" in the conventional sense. The reviews provide fresh-context
sanity checks against the drafting dialogue, which has methodological
value, but they do not provide the model-diversity that genuine
independent review implies. The author has explicitly closed the
internal Claude-review cycle at v1.2. Further substantive revision
before commitment requires human methodological review, or the
document is committed as-is.
...
**Canonical baseline implementations.** The prose descriptions above
are summaries. Material implementation details — including but not
limited to: which agents count as "opponents" in the majority
computation, whether grudge counters reset on new defections during
an active grudge, the prior used by DUM's opponent-defection-rate
model, and the response of TF2T's defection counter to interleaved
cooperation — are resolved by the harness source at
`baseline_comparison_v1.py` at the pre-registered commit hash. **The
code is canonical; the prose is summary.** Any replicator must use
the source at that hash to obtain the same baselines. Disagreement
with the implementation choices is a legitimate criticism of v1 of
the comparison and would warrant a v2 with alternative
parameterisations, not a re-run with modified baselines under this
pre-registration.
...
- `equilibrium_stabilized`: boolean — did the rolling 5-round
  ...
  between rounds 80 and 99 (the full FULL_RECOVERY phase)? This is a
  *secondary* recovery metric that captures the establishment of new
  stable equilibrium regardless of whether the equilibrium matches
  the baseline-phase cooperation rate. Added in v1.1 to handle the
  F2-observed dynamic in which altruistic constitutions recover into
  intensified cooperation (well above baseline) and cooperative
  constitutions may settle below baseline; both patterns are valid
  recoveries that the original `recovery_time` metric would record as
  `null` or misleading. Range extended in v1.2 from 80–95 to 80–99 to
  match the full FULL_RECOVERY phase.
...
*On the median/mean asymmetry between M_FP_moderate and S_FP_strict:*
The moderate tier uses median because the concern at the moderate
level is whether typical baseline-phase behaviour is quiet (the
fire-alarm test). The strict tier uses mean because the concern at
the strict level extends to outlier seeds — a framework that fires
zero baseline warnings in 49 seeds and many warnings in one seed
exhibits a specificity problem that median statistics would mask but
that mean statistics catch. The asymmetry is intentional and
reflects the different questions the two tiers ask.
...
*On stochastic operating conditions and the test design (added v1.2):*
The harness applies σ=0.08 perception noise to every Lemniscation
decision. Some readers may object that a zero-tolerance threshold
against a stochastic process is, in expectation, a probabilistic
threshold in disguise — across ~6,000 decisions per Hobbesian phase
× 50 seeds, even a very small per-decision exploit probability
produces nonzero expected exploits. This is correct but it does not
weaken the test; it specifies what the test actually tests. The
noise is not an artifact to be controlled for. It models the very
condition the framework exists to handle: real-world deployment
will involve noisy inputs and ambiguous signals, the AI equivalent
of a momentary lapse of trained reason. Training-time alignment
handles the well-trodden response; Lemniscation is supposed to be
the architecture that catches the lapse when noise produces an
impulse toward exploitation that the trained policy alone does not
intercept. Every noise-induced near-tie in the Hobbesian phase is
therefore a small instance of the out-of-distribution decision the
sentinel exists for. S5 with zero tolerance under σ=0.08 is
explicitly testing the joint claim: "across 50 seeds × ~6,000
Hobbesian decisions under σ=0.08 perception noise, the
constitutional architecture intercepts every noise-induced exploit
impulse such that no exploit action is observed." A framework that
passes this is functioning as a reliable sentinel in the conditions
it claims to handle. A framework that fails by even a small number
of exploit actions reveals that its noise-resilience is
probabilistic rather than constitutional, which is informative about
its suitability for the deployment role.
...
**Primary tests vs exploratory reporting (clarified v1.2):** The
pre-registration distinguishes two tiers of statistical activity:

*Primary tests* are the threshold checks committed in Sections 5–6
(M1–M8 plus M_FP_moderate and M7b at the moderate tier; S1–S8 plus
S_FP_strict and S7b at the strict tier). These carry the
pre-registration's inferential weight. The outcome interpretation in
Section 8 is selected based on which threshold tier the primary
tests achieve.

*Exploratory reporting* covers the full pairwise-comparison table
(Lemniscation vs each of the six baselines, across all metrics, all
phases) — approximately 150 individual comparisons. These are
reported for transparency and to allow readers to assess patterns
the threshold tests do not anchor, but they do not carry inferential
weight. Effect sizes and confidence intervals are reported for all
exploratory comparisons; p-values are reported descriptively rather
than as significance tests, and no inferential claim depends on
exploratory comparisons reaching any specific significance level.

### 7.1 Primary tests: hypothesis tests for threshold-check support

Where the threshold checks involve between-agent comparison
(specifically M6/S6 and M7/S7 against the "best baseline" pool), we
will report:

- Mean and 95% bootstrap confidence interval (10,000 resamples) for
  each agent type
- Mann-Whitney U test (two-sided) for Lemniscation vs each baseline
  in the relevant pool
- Bonferroni-corrected significance threshold for the 5-baseline
  comparison pool (after AC exclusion): α = 0.05 / 5 = 0.01 per
  individual comparison

These primary tests support the threshold checks in Sections 5–6;
they do not establish independent inferential claims.

### 7.2 Effect sizes (reported for both primary and exploratory)
...
methodological consensus). For exploratory comparisons, effect sizes
are reported descriptively.

### 7.3 Reporting

A single results table will report **all comparisons**, primary and
exploratory, across all metrics, all phases, all six baselines. The
table will be clearly partitioned: primary comparisons (those
referenced by Section 5–6 threshold checks) will be visually marked;
the remaining comparisons will be marked as exploratory and stated as
non-inferential. No metric will be omitted from reporting based on
results inspection. The pre-registered threshold check outcomes
(Sections 5–6 → Section 8 outcome paragraph) constitute the
pre-registration's primary finding.
...
criteria and in the homogeneous-protagonist condition tested, that
Lemniscation v5.9's constitutional architecture produces behavioural
and diagnostic patterns that are not replicable in the same condition
by simple reciprocity strategies, generous reciprocity variants,
naïve cooperation, discounted utility maximization, or
fixed-disposition agents with grudge mechanisms. The framework's
central empirical claims — zero cooperative exploiting under sustained
adversarial pressure, reliable early drift detection with meaningful
lead time over observable behavioural change, specific
(low-false-positive) detection signals, and recovery dynamics
competitive with the fastest baselines — survive direct comparison at
the strict tier in the v1 comparison. The mixed-protagonist condition,
in which Lemniscation must extend the self to include genuinely
different agents rather than structurally identical copies, is not
tested in v1; a v2 comparison addressing this is the natural follow-on
and the philosophical claim of extensible-self is more directly tested
there. Unresolved limitations identified in the white paper
(corrigibility, multi-constitution conflict, real-LLM deployment)
remain the priority for subsequent work; these comparison results do
not address them. The framework now merits investment in those
...
### 8.5 Procedural commitment on Step 2

Added in v1.2. The author commits to the following procedural rule
governing the design of Step 2 of the strategic plan:

- The design of Step 2 will be specified in a separate pre-registration
  document, committed to git before any Step 2 data is generated, and
  registered as a new OSF entry referencing this Step 1 pre-registration
  and its results.
- The scope and design of Step 2 will be constrained by Step 1's
  actual outcome (A, B, C, or D). Broadly: under Outcome A, Step 2
  appropriately moves toward addressing white-paper limitations
  (mixed-protagonist comparison, real-LLM bridge); under Outcome B,
  Step 2 prioritises the specific architectural components implicated
  by the strict-tier failures; under Outcome C, Step 2 isolates the
  partial-function signals identified and tests them directly; under
  Outcome D, the current strategic plan is suspended pending
  fundamental architectural review.
- This is a procedural commitment, not a detailed design commitment.
  Specific Step 2 design choices will be informed by what Step 1
  actually shows and will be pre-registered in detail in their own
  document. Deviation from the outcome-conditional scope constraint
  above requires explicit justification in the Step 2 pre-registration.
...
**Stopping rule (added v1.2):** The full 350-run sweep executes to
completion or to unrecoverable error. The harness is structured such
that per-seed results are not surfaced to the author until the full
sweep completes (or fails). If the sweep fails partway:

- The partial results are committed to git AS-IS, with the harness's
  `__meta__` field reporting `sweep_status: "partial_due_to_error"`,
  the error type, and which seeds were completed for each agent type.
- The failure is documented in a brief commit message.
- Any re-run after a partial completion is treated as a separate
  experiment requiring a new pre-registration amendment that
  explicitly records what was already observed at the time of the
  amendment.
- No per-seed inspection of partial results occurs before the
  amendment is committed.

This forecloses the researcher degree of freedom that would otherwise
exist: a partial sweep cannot be silently restarted with knowledge of
which seeds produced which results.

**Harness self-verification (added v1.2):** The harness writes its
own git commit hash into the `__meta__` field of every output JSON
(both `--smoke` mode and `--execute` mode). The hash is read from
the live git repository the harness is sitting in, and includes a
verification that the working copy is not modified relative to HEAD.
Any results file whose `harness_git_hash` does not match the
pre-registered hash, or whose hash starts with `UNVERIFIED_`, is
rejected by downstream analysis and does not constitute a valid
results-of-the-pre-registered-comparison. This forecloses the
researcher degree of freedom that would otherwise exist: a results
file cannot be silently produced by a modified harness and presented
as the pre-registered comparison.

7. The raw results JSON is committed to git **immediately** upon
   completion (or upon unrecoverable error), before any analysis.
8. Statistical analysis is performed per Section 7, threshold checks
...
9. Results, analysis, and the selected outcome paragraph are committed
...
(Anthropic), via interactive dialogue. Conversation logs available
on request.

**Review process:** Two rounds of separate-instance review by
additional Claude instances (Anthropic), without access to the
drafting dialogue, conducted before any results-generating run was
executed. Round 1 (v1.0 → v1.1) surfaced six substantive critiques.
Round 2 (v1.1 → v1.2) surfaced eleven additional critiques and the
explicit observation that the review process itself was overclaiming
"independence" when both reviewers shared a model with the drafter.

**Limitation of the review process:** While different Claude
instances do not share state, they share the same model weights,
training data, and inductive biases. The reviews provide
fresh-context sanity checks against the drafting dialogue, which has
methodological value, but they do not provide the model-diversity
that independent peer review in the conventional academic sense
implies. Genuine independent review by a human methodologist or by
non-Claude reviewers would strengthen the document materially and
...
prior rounds surfacing 6 and 11 critiques respectively suggests
diminishing returns and risk of indefinite refinement. Any further
substantive revision before commitment requires human methodological
review, or the document is committed as-is.
...
### v1.2 (this version)
Substantive changes from v1.1 in response to a second round of
separate-instance Claude review:

1. **Section 2 and Section 11 rephrased**: "independent review" usage
   replaced with "separate-instance review" throughout, with explicit
   acknowledgement that same-model reviewers do not provide the
   model-diversity that conventional independent peer review implies.
   The review transcripts are now committed to the repository for
   auditability. Rationale: the v1.1 framing overclaimed independence.

2. **Stopping rule added (Section 10)**: full 350-run sweep executes
   to completion or unrecoverable error; partial results committed
   as-is; any re-run requires new amendment recording prior
   observations. Rationale: closes the researcher-degree-of-freedom
   loophole that would otherwise allow silent restart of a partial
   sweep with knowledge of completed seeds.

3. **Harness self-verification added (Section 10)**: harness writes
   its own git commit hash into every output JSON `__meta__` field,
   with verification that the working copy is unmodified relative to
   HEAD. Results files lacking a matching hash are rejected by
   downstream analysis. Implemented in the harness source at the
   same commit. Rationale: closes the loophole that would otherwise
   allow results to be produced by a modified harness.

4. **Baseline implementations canonicalised to harness source
   (Section 3.1)**: explicit commitment that the prose descriptions
   are summaries and the harness source at the pre-registered commit
   hash is canonical. Rationale: forecloses post-hoc disputes about
   ambiguous prose descriptions while preserving the right to
   parameterise differently in a future v2.

5. **Primary vs exploratory testing distinction added (Section 7)**:
   the threshold checks (Sections 5–6) are designated primary and
   carry inferential weight; the ~150 pairwise comparisons in the
   full reporting table are designated exploratory with no
   inferential claims attached. Bonferroni correction is recalibrated
   to the 5-baseline comparison pool (after AC exclusion) at α = 0.01
   for primary tests. Rationale: the v1.1 framing risked confusing
   the 6-comparison Bonferroni correction with the 150-comparison
   reporting table, which would have been vastly under-corrected.

6. **Step 2 procedural commitment added (Section 8.5)**: a procedural
   rule governing how Step 2's design is constrained by Step 1's
   actual outcome. Rationale: forecloses the degree of freedom that
   would otherwise let Step 2 be framed to favour whatever Step 1
   produced, without overcommitting to a detailed Step 2 design
   in advance.

7. **Outcome A qualified for homogeneous-protagonist scope
   (Section 8.1)**: "are not replicable by..." became "are not
   replicable in the homogeneous-protagonist condition tested by...".
   An explicit note about the mixed-protagonist condition not being
   tested in v1 was added to the outcome paragraph. Rationale: the
   extensible-self mechanism's core philosophical claim is most
   directly tested in the mixed condition; the v1 comparison tests
   the degenerate case and Outcome A should not overclaim.

8. **S5 stochastic-process clarification added (Section 6)**:
   explicit characterisation of what zero-tolerance under σ=0.08
   perception noise actually tests. The noise is the test condition,
   not an artifact; every noise-induced near-tie is a small instance
   of the out-of-distribution decision the sentinel exists for; S5
   tests whether the constitutional architecture intercepts every
   such impulse. Rationale: closes the ambiguity about what
   "= 0.000" means under a stochastic process while preserving the
   threshold as a stern test of the framework's noise-resilience
   claim.

9. **M_FP_moderate / S_FP_strict statistic asymmetry justified
   (Section 5)**: explanation added for why moderate uses median and
   strict uses mean. Rationale: the asymmetry is intentional but was
   not justified in v1.1.

10. **equilibrium_stabilized range extended (Section 4.1)**: changed
    from rounds 80–95 to rounds 80–99 to match the full FULL_RECOVERY
    phase. Rationale: the original range cut off the last 4 rounds
    without justification.

11. **Section 7 wording clarified**: "six pairwise comparisons"
    ambiguity resolved by the primary/exploratory partition.
    Rationale: minor housekeeping flagged by review.

### v1.1 (superseded)
Substantive changes from v1.0 in response to first round of
separate-instance Claude review:
...
12. **Section 2 expanded**: the separate-instance review process and
    the six critiques it surfaced are documented as part of the
